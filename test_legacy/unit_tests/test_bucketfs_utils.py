...
def test_generate_bucket_udf_path_non_archive_file():
    ...
    udf_path = bucketfs_utils.generate_bucket_udf_path(
        bucket_config=bucket_config, path_in_bucket="path/in/bucket/test_file.txt"
    )
...
def test_generate_bucket_url_file_write_access():
    ...
    assert (
        udf_path.geturl()
        == "http://localhost:6666/default/path/in/bucket/test_file.txt"
    )
...
def test_generate_bucket_url_file_with_credentials():
    ...
    assert (
        udf_path.geturl()
        == "http://w:write@localhost:6666/default/path/in/bucket/test_file.txt"
    )


def test_generate_bucket_url_file_with_ip():
    connection_config = BucketFSConnectionConfig(
        host="127.0.0.1", port=6666, user="w", pwd="write", is_https=False
    )
    bucketfs_config = BucketFSConfig(
        connection_config=connection_config, bucketfs_name="bfsdefault"
    )
    ...
    assert (
        udf_path.geturl()
        == "http://w:write@127.0.0.1:6666/default/path/in/bucket/test_file.txt"
    )


def test_generate_bucket_url_file_with_whitespace_in_host():
    connection_config = BucketFSConnectionConfig(
        host="local host", port=6666, user="w", pwd="write", is_https=False
    )
    bucketfs_config = BucketFSConfig(
        connection_config=connection_config, bucketfs_name="bfsdefault"
    )
    ...
    assert (
        udf_path.geturl()
        == "http://w:write@local%20host:6666/default/path/in/bucket/test_file.txt"
    )


def test_generate_bucket_url_file_with_whitespace_in_password():
    connection_config = BucketFSConnectionConfig(
        host="localhost", port=6666, user="w", pwd="write write", is_https=False
    )
    bucketfs_config = BucketFSConfig(
        connection_config=connection_config, bucketfs_name="bfsdefault"
    )
    ...
    assert (
        udf_path.geturl()
        == "http://w:write%20write@localhost:6666/default/path/in/bucket/test_file.txt"
    )
...
def test_generate_bucket_url_file_with_whitespace_in_bucket_name():
    ...
    bucket_config = BucketConfig(
        bucket_name="default default", bucketfs_config=bucketfs_config
    )
    udf_path = bucketfs_utils.generate_bucket_http_url(
        bucket_config=bucket_config,
        path_in_bucket="path/in/bucket/test_file.txt",
    ...
    assert (
        udf_path.geturl()
        == "http://w:write@localhost:6666/default%20default/path/in/bucket/test_file.txt"
    )
...
def test_generate_bucket_url_file_with_whitespace_in_path_in_bucket():
    ...
    assert (
        udf_path.geturl()
        == "http://w:write@localhost:6666/default/path/in/bucket/test%20file.txt"
    )


def test_generate_bucket_url_file_read_only_access():
    connection_config = BucketFSConnectionConfig(
        host="localhost", port=6666, user="r", pwd="read", is_https=False
    )
    bucketfs_config = BucketFSConfig(
        connection_config=connection_config, bucketfs_name="bfsdefault"
    )
    ...
    assert (
        udf_path.geturl()
        == "http://r:read@localhost:6666/default/path/in/bucket/test_file.txt"
    )


def test_generate_bucket_url_file_https():
    connection_config = BucketFSConnectionConfig(
        host="localhost", port=6666, user="r", pwd="read", is_https=True
    )
    bucketfs_config = BucketFSConfig(
        connection_config=connection_config, bucketfs_name="bfsdefault"
    )
    ...
    assert (
        udf_path.geturl()
        == "https://r:read@localhost:6666/default/path/in/bucket/test_file.txt"
    )
