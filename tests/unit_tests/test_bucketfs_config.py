...
def test_bucketfs_config_with_bucketfs_connection_config():
    ...
    bucketfs_config = BucketFSConfig(
        bucketfs_name=bucketfs_name, connection_config=connection_config
    )

    assert (
        bucketfs_config.bucketfs_name == bucketfs_name
        and bucketfs_config.connection_config == connection_config
    )
...
def test_bucketfs_config_without_bucketfs_connection_config():
    ...
    assert (
        bucketfs_config.bucketfs_name == bucketfs_name
        and bucketfs_config.connection_config == None
    )
