...
from exasol_bucketfs_utils_python.bucket_config import (
    BucketConfig,
    BucketFSConfig,
)
...
@pytest.mark.usefixtures(
    "upload_language_container", "pyexasol_connection", "bucketfs_location"
)
...
def upload_testfile_to_bucketfs(
    bucket_config: BucketConfig, file_path: str, content: str
):
...
def test_load_file_to_string(
    ...
    try:
        ...
        delete_testfile_from_bucketfs(
            file_path=path_in_bucket, bucket_config=bucketfs_location.bucket_config
        )
        pyexasol_connection.execute(f"DROP SCHEMA IF EXISTS {target_schema} CASCADE;")


def test_load_file_to_fileobj(
    upload_language_container, pyexasol_connection, bucketfs_location
):
...
def test_load_file_to_object_via_joblib(
    upload_language_container, pyexasol_connection, bucketfs_location
):
